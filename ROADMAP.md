...
| [Value clips](https://openusd.org/release/api/_usd__page__value_clips.html) | `12.3.4` | :white_check_mark: | `main` | Explicit + template clips resolved during value resolution: parsing, manifest gating, active-clip selection, stage-to-clip time mapping (incl. jumps), clip strength below local opinions, with a full-parity `ClipsAPI` read/write view<br>Template clips (§12.3.4.1.3) expand to explicit sets, retimed by layer offset; `interpolateMissingClipValues` + missing-value sentinels<br>Manifest generation in `pcp::clip_manifest`, reached through `usd::ClipsAPI::generate_clip_manifest`; a set that authors no `manifestAssetPath` resolves through one synthesized from its clips, so every set is gated alike (§12.3.4.1.1.2)<br>`Attribute::time_sample_times` reports every clip's activation time alongside its in-window samples, isolating a clip from its neighbours (C++ `Usd_Clip::ListTimeSamplesForPath`) |
...
| Feature | Spec | Status | Version | Notes |
|---|---|---|---|---|
| [Schema registry](https://openusd.org/release/api/class_usd_schema_registry.html) | `13.3` | :construction: | | `usd::SchemaRegistry` built from `usd::FamilySource` pairs (flattened schematics + manifest), with `usd::SchemaInfo`, `usd::PrimDefinition`, and a `usd::PrimTypeInfo` interner; per-stage via `usd::StageBuilder::schema_registry`<br>Remaining — import the OpenUSD schema data |
| [Typed schemas](https://openusd.org/release/api/class_usd_typed.html) (IsA) | `13.3.1` | :white_check_mark: | `main` | A concrete type's registered definition backs `usd::Prim::prim_definition`, so its properties, fallbacks and built-in API schemas resolve<br>`usd::Prim::is_a` answers against the whole base chain (`SchemaRegistry::is_a` walks `SchemaInfo::bases`, resolving `fallbackPrimTypes` through `PrimTypeInfo::schema_type_name`), and the typed views' `get` gate accepts a subtype through it. Both sides of a query must name a registered schema, as in C++<br>Family queries go through `usd::VersionFilter`, `usd::SchemaRegistry::schema_infos_in_family` and `usd::Prim::is_in_family`<br>Remaining — the gate falls back to the authored `typeName` for a prim whose type the stage's registry has no schema for, which is every prim until the schema data is imported |
| [Applied schemas](https://openusd.org/release/api/class_usd_a_p_i_schema_base.html) (HasA) | `13.3.2` | :white_check_mark: | `main` | `apiSchemas` list-ops compose across layers; a prim's composed list drives `SchemaRegistry::build_composed_prim_definition` through `usd::Prim::prim_definition`<br>Application is validated by `usd::Prim::apply_api` and `can_apply_api` over `SchemaRegistry::check_applied_name` and `is_allowed_instance_name`, reporting `usd::ApplyApiError`; `add_applied_schema` stays the unchecked primitive<br>`usd::Prim::has_api_in_family` answers a versioned family, over `usd::VersionFilter` |
...
|---|---|---|---|
| [Variable expressions](https://openusd.org/release/api/_sdf__page__variable_expressions.html) | :white_check_mark: | `0.2.0` | 13 built-in functions, string interpolation (`expr` module); session and root layer `expressionVariables` compose together (C++ `PcpExpressionVariables`, `expr::stack_expression_variables`), so a session opinion resolves the root's `${VAR}` sublayers; a `${VAR}` sublayer a runtime edit newly selects loads on demand |
...
| [UsdShade](https://openusd.org/release/api/usd_shade_page_front.html) (materials, shaders) | :white_check_mark: | `0.5.0` | Trait-views behind `shade` (read + author): `Connectable`, `Input`, `Output`, `ConnectedSources`, `ShadingAttribute`, `ResolvedTerminal`, `InterfaceInputConsumersMap`, Shader / NodeGraph / Material, MaterialBindingAPI, and the UsdPreviewSurface reader<br>Remaining — Material base-material; `CoordSysAPI`; renderer shader dialects (MDL / MaterialX) |
...
| [Namespace editing](https://openusd.org/release/api/class_usd_namespace_editor.html) | :white_check_mark: | `0.6.0` | [`usd::NamespaceEditor`](src/usd/editor.rs): batched, atomic rename / reparent / delete with target and reference fixup, authoring relocates for cross-arc content |
...
| [Change notification](https://openusd.org/release/api/class_usd_notice.html) | :white_check_mark: | `0.6.0` | `sdf::LayerSink` (layer commit seam) and `usd::StageSink` (composed changes); transferable `Diff` via `usd::UndoStage` / `usd::ReplayStage` and `Stage::apply_diff` |
