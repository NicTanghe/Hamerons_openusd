...
use super::{AttributeType, Connectable, Output, ShadingAttribute};
...
impl Material {
    ...
    fn resolve_terminal(&self, kind: TerminalKind, render_context: &str) -> Result<Option<ResolvedTerminal>> {
        ...
        for attribute in output.value_producing_attributes(true)? {
...
mod tests {
    ...
    fn material_surface_terminal() -> Result<()> {
        ...
        let surface = mat.compute_surface_source(&[])?.expect("surface terminal");
        assert_eq!(surface.kind(), TerminalKind::Surface);
        assert!(surface.render_context().is_empty());
        assert_eq!(surface.sources().len(), 1);
        assert_eq!(surface.sources()[0].shader().path().as_str(), "/Mat/Surface");
        assert_eq!(surface.sources()[0].source_name(), "surface");
        assert_eq!(surface.sources()[0].source_type(), AttributeType::Output);
        Ok(())
    }
    ...
    fn material_terminal_kinds() -> Result<()> {
        ...
        material.create_surface_output()?.connect_to_output(&surface)?;
        material
            .create_displacement_output()?
            .connect_to_output(&displacement)?;
        material.create_volume_output()?.connect_to_output(&volume)?;
    ...
    fn context_selection_fallback() -> Result<()> {
        ...
        let material = Material::define(&stage, "/Mat")?;
        material.create_surface_output()?.connect_to_output(&universal)?;
        material
            .create_surface_output_for("mtlx")?
            .set_connections([sdf::path("/Missing.outputs:result")?])?;
        material
            .create_surface_output_for("ri")?
            .connect_to_output(&renderman)?;
    ...
    fn terminal_source_order() -> Result<()> {
        ...
        material
            ...
            .connect_to_output(&graph.output("volume"))?;
